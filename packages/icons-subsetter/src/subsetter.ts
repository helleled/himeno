...
export async function generateSubsettedFont(ttfPath: string, unicodeRangeValues: Map<string, number[]>) {
    ...
    const { instance } = await WebAssembly.instantiate(await fsp.readFile('./node_modules/harfbuzzjs/hb-subset.wasm'));
    const exports = instance.exports as HarfbuzzWasm;

    // Verify required Harfbuzz exports exist
    if (!exports.memory) {
        throw new Error('WebAssembly module is missing required "memory" export');
    }
    if (!exports.hb_subset_input_create_or_fail || 
        !exports.malloc || 
        !exports.hb_blob_create || 
        !exports.hb_face_create) {
        throw new Error('WebAssembly module is missing required Harfbuzz function exports');
    }

    const harfbuzzWasm = exports;
