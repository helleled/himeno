...
describe('UserEntityService', () => {
    describe('pack/packMany', () => {
        ...
        describe('packManyによるpreloadがある時、preloadが無い時とpackの結果が同じになるか見たい', () => {
            ...
            test('pack remote avatar decorations', async () => {
                const user = await createUser({
                    host: 'remote.example.com',
                    uri: 'https://remote.example.com/users/test',
                    avatarDecorations: [
                        {
                            id: 'remote-deco-1',
                            url: 'https://remote.example.com/decorations/deco1.png',
                            angle: 30,
                            flipH: true,
                            offsetX: 0.1,
                            offsetY: -0.2,
                ...
                const packed = await service.pack(user, null, { schema: 'UserLite' });
                expect(packed.avatarDecorations).toHaveLength(1);
                expect(packed.avatarDecorations[0].id).toBe('remote-deco-1');
                expect(packed.avatarDecorations[0].url).toBe('https://remote.example.com/decorations/deco1.png');
                expect(packed.avatarDecorations[0].angle).toBe(30);
                expect(packed.avatarDecorations[0].flipH).toBe(true);
                expect(packed.avatarDecorations[0].offsetX).toBe(0.1);
                expect(packed.avatarDecorations[0].offsetY).toBe(-0.2);
            });
        });
    });
});
