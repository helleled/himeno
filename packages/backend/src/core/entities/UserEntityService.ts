...
export class UserEntityService implements OnModuleInit {
    ...
    public async pack<S extends 'MeDetailed' | 'UserDetailedNotMe' | 'UserDetailed' | 'UserLite' = 'UserLite'>(
        ...
        const packed = {
            ...
            avatarDecorations: user.avatarDecorations.length > 0 ? this.avatarDecorationService.getAll().then(decorations => user.avatarDecorations.map(ud => {
                const decoration = decorations.find(d => d.id === ud.id);
                if (decoration) {
                    return {
                        id: ud.id,
