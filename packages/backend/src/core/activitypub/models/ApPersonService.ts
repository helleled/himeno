...
export class ApPersonService implements OnModuleInit {
    ...
    /**
     * Sanitize avatar decorations from remote actors
     */
    @bindThis
    private sanitizeAvatarDecorations(person: IActor, host: string): MiUser['avatarDecorations'] | undefined {
        if (!person._misskey_avatarDecorations || !Array.isArray(person._misskey_avatarDecorations)) {
            return undefined;
        }

        const maxDecorations = 16;
        const decorations = person._misskey_avatarDecorations
            .slice(0, maxDecorations)
            .map(decoration => {
                if (!decoration || typeof decoration !== 'object') return null;
                if (typeof decoration.id !== 'string' || !decoration.id) return null;
                if (typeof decoration.url !== 'string' || !decoration.url) return null;

                try {
                    const url = new URL(decoration.url);
                    if (url.protocol !== 'https:') return null;
                    if (this.utilityService.punyHost(decoration.url) !== host) return null;
                } catch {
                    return null;
                }

                const sanitized: NonNullable<MiUser['avatarDecorations'][number]> = {
                    id: decoration.id.slice(0, 512),
                    url: decoration.url.slice(0, 2048),
                };

                if (typeof decoration.angle === 'number' && isFinite(decoration.angle)) {
                    sanitized.angle = Math.max(-360, Math.min(360, decoration.angle));
                }

                if (typeof decoration.flipH === 'boolean') {
                    sanitized.flipH = decoration.flipH;
                }

                if (typeof decoration.offsetX === 'number' && isFinite(decoration.offsetX)) {
                    sanitized.offsetX = Math.max(-1, Math.min(1, decoration.offsetX));
                }

                if (typeof decoration.offsetY === 'number' && isFinite(decoration.offsetY)) {
                    sanitized.offsetY = Math.max(-1, Math.min(1, decoration.offsetY));
                }
    ...
    public async createPerson(uri: string, resolver?: Resolver): Promise<MiRemoteUser> {
        ...
        const avatarDecorations = this.sanitizeAvatarDecorations(person, host);
        //#endregion
    ...
    public async updatePerson(uri: string, resolver?: Resolver | null, hint?: IObject, movePreventUris: string[] = []): Promise<string | void> {
        ...
        // アバターデコレーション取得
        const avatarDecorations = this.sanitizeAvatarDecorations(person, exist.host);
        ...
        const updates = {
            ...
            isExplorable: person.discoverable,
            ...(avatarDecorations !== undefined ? { avatarDecorations } : {}),
